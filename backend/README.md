...
The pipeline loops through every section and asks the model: *"Does this section contain concepts that would benefit from a Manim visualization?"*

**Filtering (before sending to the model):**
- Skips sections titled "References", "Bibliography", "Acknowledgments", "Related Work"
- Skips sections shorter than 100 characters

**What the model returns (per section):**
```json
{
...
For each candidate, the model creates a scene-by-scene storyboard:
...
The model writes complete, runnable Manim Python code. Key features:
...
This dramatically improves code quality because the model sees a working pattern to follow.
...
This is sent to `ManimGenerator.run_with_feedback()` along with the previous broken code, so the model can see exactly what went wrong and fix it.
...
```python
DEFAULT_MODEL = "gpt-5.4"             # OpenAI Responses API default model
```
