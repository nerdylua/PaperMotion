# arXivisual Agent Context

arXivisual turns papers into interactive visual explanations: ingest a paper, summarize/structure it, identify visualizable concepts, generate 3Blue1Brown-style Manim scenes with optional voiceover, validate/render videos, and display them in a Next.js scrollytelling reader.

## Repo Shape

- `frontend/`: Next.js 16 + React 19 App Router UI. Main routes are `/`, `/abs/[...id]`, and `/topic`.
- `backend/`: FastAPI + async SQLAlchemy backend. Handles ingestion, LLM agents, validation, Manim rendering, storage, and job status.
- `features.md`: best high-level product map. Prefer it over older aspirational docs when orienting.
- Root `docker-compose.yml` runs the backend only; run/deploy the frontend separately.
- Generated/local artifacts: do not commit `.env`, `.env.local`, `.venv/`, `.next/`, `node_modules/`, `backend/arxiviz.db`, `*.sqlite3`, `backend/media/`, `backend/generated_output/`, `server/outputs/*`, or rendered MP4s.
...
- DB defaults to SQLite at `backend/arxiviz.db`; `DATABASE_URL` switches to async Postgres URL forms in `backend/db/connection.py`.
...
- API contract:
  ...
  - `GET /api/video/{video_id}`: local file or R2 redirect.
...
2. Generation pipeline (`backend/agents/pipeline.py`):
   ...
   - Optional `M2M2PlanningLayer` adds paper-grounded scene specs; failure falls back to the base plan.
   - `ManimGenerator` selects few-shot examples by `VisualizationType`, fetches Manim docs via Context7/OpenAI if possible, and emits Manim code. Unified voice mode emits `VoiceoverScene` with gTTS blocks.
...
3. Rendering/storage:
   ...
   - `RENDER_MODE=modal` calls deployed Modal function `arxiviz-manim/render_manim_modal`.
   - `STORAGE_MODE=local` writes `./media/videos`; `STORAGE_MODE=r2` uploads to Cloudflare R2.
...
- Config: strict TypeScript, `@/*` path alias, Next ESLint core web vitals/typescript, Tailwind v4 via `@tailwindcss/postcss`.
- Quirks: `axios` and `@tanstack/react-query` are installed but current handwritten API code uses `fetch` and local hooks. `paper-source.ts` and `pdf-id.ts` appear unused by active routes. Some metadata/demo references (`/icon.png`, `/landing.jpeg`, `/videos/demo/*.mp4`) may point to public assets not present in this checkout.

## Runtime Config

- Backend required: `OPENAI_API_KEY`.
- Backend common optional: `OPENAI_MODEL`, `OPENAI_CONTEXT7_MODEL`, `OPENAI_REASONING_EFFORT`, `OPENAI_BASE_URL`, `OPENAI_EMBEDDING_MODEL`, `RENDER_MODE=local|modal`, `STORAGE_MODE=local|r2`, `RENDER_CONCURRENCY`, `RENDER_TIMEOUT_SECONDS`, `MANIM_SUBPROCESS_TIMEOUT_SECONDS`, `MIKTEX_BIN_DIR`, arXiv throttle env vars.
...
- Offline-ish pipeline smoke: `cd backend && uv run python tools/test_pipeline.py`.
- Full pipeline/dev rendering scripts live under `backend/tools/` and `backend/tools/pipeline-tests/`; run from `backend/`.
...
- For new visualization types, update `VisualizationType`, add a few-shot example in `backend/examples/`, map it in `ManimGenerator.EXAMPLE_FILES` and `VOICEOVER_EXAMPLE_FILES`, and adjust prompts/tests.
...
- Only change lockfiles (`frontend/package-lock.json`, `backend/uv.lock`) when intentionally updating dependencies.
- `tmp_job_diag.py` and `backend/manimations/` are diagnostic/stub areas, not core product paths.
